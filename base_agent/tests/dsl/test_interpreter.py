...
class TestDslInterpreter:
    ...
    def test_parse_valid_string(self, interpreter):
        dsl_string = "IF SMA(10) > SMA(50) THEN BUY ELSE SELL"
        program = interpreter.parse(dsl_string)
        assert program is not None
        assert len(program) == 1
        rule = program[0]
        assert rule.condition.indicator1 == Indicator.SMA
        assert rule.condition.param1 == 10
        assert rule.condition.operator == Operator.GREATER_THAN
        assert rule.condition.indicator2 == Indicator.SMA
    ...
    @pytest.mark.parametrize("invalid_string", [
        "IF SMA(10) > SMA(50) THEN BUY",  # Missing ELSE
        "IF SMA(10) > SMA(50) SELL ELSE BUY", # Missing THEN
        "FOO SMA(10) > SMA(50) THEN BUY ELSE SELL", # Wrong keyword
        "IF FAKE(10) > SMA(50) THEN BUY ELSE SELL", # Invalid indicator
        "IF SMA(10) @ SMA(50) THEN BUY ELSE SELL", # Invalid operator
        "IF SMA(10) > SMA(50) THEN JUMP ELSE CROUCH", # Invalid action
    ])
